...
//    INTX_DISABLE();                         //关闭所有中断，复位过程不能被打断！
    HAL_GPIO_WritePin(ETH_nRST_GPIO_Port, ETH_nRST_Pin,GPIO_PIN_RESET);//硬件复位
//    vTaskDelay(100);
	HAL_Delay(100);
	HAL_GPIO_WritePin(ETH_nRST_GPIO_Port, ETH_nRST_Pin,GPIO_PIN_SET);//复位结束
//    vTaskDelay(100);
	HAL_Delay(100);
//    INTX_ENABLE();                          //开启所有中断  
	...
	MACAddr[0] = 0x02;			//高三字节(IEEE称之为组织唯一ID,OUI)地址固定为:2.0.0
	MACAddr[1] = 0x00;
	MACAddr[2] = 0x00;
	...
	myprintf("\r\n MAC:%d-%d-%d-%d-%d-%d",MACAddr[0],MACAddr[1],MACAddr[2],\
    ...
    heth.Init.PhyAddress=LAN7820A_PHY_ADDRESS;//LAN8720地址  
...
/* Init ETH */
  ...
  heth.Init.PhyAddress = LAN7820A_PHY_ADDRESS;
  MACAddr[0] = 0x02;			//高三字节(IEEE称之为组织唯一ID,OUI)地址固定为:2.0.0
  MACAddr[1] = 0x00;
  MACAddr[2] = 0x00;
  MACAddr[3] = (sn0>>16) & 0xFF;//低三字节用STM32的唯一ID
