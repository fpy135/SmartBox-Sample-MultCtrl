...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    if(huart->Instance == USART1)	// 判断是由哪个串口触发的中断
    {
		...
		sta = HAL_UART_Receive_IT(&huart1, (uint8_t *)&aRxBuffer, 1);   //再开启接收中断
		if(sta != HAL_OK)
		{
			if(sta == HAL_BUSY)
			{
	...
	if(huart->Instance == USART2)	// 判断是由哪个串口触发的中断
    {
		...
		sta = HAL_UART_Receive_IT(&huart2, (uint8_t *)&aRxBuffer, 1);   //再开启接收中断
		if(sta != HAL_OK)
		{
			if(sta == HAL_BUSY)
			{
	...
	else if(huart->Instance == USART3)
	{
		...
		sta = HAL_UART_Receive_IT(&huart3, (uint8_t *)&aRxBuffer, 1);   //再开启接收中断
		if(sta != HAL_OK)
		{
			if(sta == HAL_BUSY)
			{
	...
	else if(huart->Instance == USART6)
	{
		...
		sta = HAL_UART_Receive_IT(&huart6, (uint8_t *)&aRxBuffer, 1);   //再开启接收中断
		if(sta != HAL_OK)
		{
			if(sta == HAL_BUSY)
			{
