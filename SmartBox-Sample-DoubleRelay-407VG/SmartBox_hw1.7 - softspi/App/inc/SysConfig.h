...
7. 2022/09/05	sw:2.0.20	hw：2.0.0改版，sw：变更为2.*.*
				优化FLASH耗电量的存储方式，降低FLASH擦除次数，延长flash使用寿命
				优化webserver，ID兼容16进制，新增版本号，电压电流等电量计

				优化时间策略00:00上电后等不亮情况
8. 2022/09/28	新增基于485的倾角传感器和智能空开
				并将传感器的使用选择置于webserver上
9. 2022/11/14	新增基于耘农十合一传感器的负氧离子上报
10. 2022/11/22	sw2.0.22
				修复电量计通讯失败后，重新从flash读取数据后全ffffffff的隐患，新增长时间未接收到数据后整机重启逻辑
12. 2022/12/26	sw2.0.23
				将耘农温湿度的485地址改为0x15
13. 2022/12/31	修复上电继电器状态不稳定
				修复一年中最后天的uinx时间戳转换错误问题
14. 2023/01/03	sw:2.0.24
				修复倾角协议未更新问题新增有效位
15. 2023/01/10  sw:2.0.25
				新增远程更改服务器地址
				修复模拟井盖未触发
				网页新增倾角、井盖、水位的展示
***********************************************/
...
#define SOFTWARE_VERSION		0x2019		//v2.0.25
...
extern SemaphoreHandle_t AirSwitchBinary;	//智能空开信号量
extern SemaphoreHandle_t AngleBinary;		//倾角信号量

extern SemaphoreHandle_t ManholeCoverBinary;		//井盖信号量
...
extern QueueHandle_t TM_AirSwitch_Queue;	//智能空开队列
