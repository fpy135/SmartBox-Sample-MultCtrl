...
uint8_t *Protocol_Pack_2(uint8_t sn, uint16_t type, uint32_t id, uint8_t cmd,uint16_t len, uint8_t *content)
{
    uint16_t CRCRes;
    uint8_t *pBuf;
	uint16_t temp_type = HALFWORD_Reverse(type);
    ...
    send_data[4]	= (uint8_t)((temp_type>>8) & 0xff);
    send_data[5]  	= (uint8_t)(temp_id & 0xff);
    send_data[6]	= (uint8_t)((temp_id>>8) & 0xff);
    send_data[7]   	= (uint8_t)((temp_id>>16) & 0xff);
    send_data[8]  	= (uint8_t)((temp_id>>24) & 0xff);
    send_data[9]   	= cmd;
    send_data[10]  	= len;
    memcpy(&send_data[11], content, len);

    pBuf = (uint8_t *)(&send_data);
    CRCRes = CRC16_calc(pBuf, 11+len);
//    RFProtocol.crc = (CRCRes << 8) | (CRCRes >> 8);
    send_data[12+len] = (uint8_t)(CRCRes & 0xff);
    send_data[13+len+1] = (uint8_t)((CRCRes>>8) & 0xff);
    return pBuf;
}
