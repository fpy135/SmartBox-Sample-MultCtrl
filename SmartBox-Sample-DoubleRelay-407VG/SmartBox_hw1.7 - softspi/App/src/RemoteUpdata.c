...
void getMD5(uint8_t *md5_32,uint8_t *md5_16)
{
	...
	for(i = 0;i<32;i=i+2)
	{
		md5_16[i/2] = (md5_32[i] - '0')<<8 | (md5_32[i+1] - '0')<<8;
	}
}
...
uint8_t RemoteUpdataBy4G(uint8_t *buf, uint16_t len)
{
...
//		UART3Write(buf,len);
			...
			W25QXX_Erase_NSector(UPDATA_ADDR,CODE_SIZE/W25XX_SECTOR_SIZE); // 擦除更新区
		#else
	...
	{
		#if USE_OFFCHIP_FLASH
			W25QXX_Write(buf, UPDATA_ADDR+buff_len, len);
		#else
...
//		RemoteUpdata_ACK();
		myprintf("\r\nbuff_len:%d",buff_len);
		if(buff_len == file_len)		//接收完成所有数据包
		{
			uint8_t i=0;
			uint8_t calculate_md5[16] = {0};
			uint8_t temp_buff[CODE_INFO_LEN];
			
			updata_flag = 0;	//更新标志结束
			myprintf("\r\n 计算更新区内的程序MD5：");
			CheckCodeMD5_Boot(UPDATA, file_len, calculate_md5);
			for(i=0;i<16;i++)
			{
			...
			{
				myprintf("\r\n MD5校验通过");
				temp_buff[CODE_LEN_ADDR] =   (buff_len>>24)&0xff;
				temp_buff[CODE_LEN_ADDR+1] = (buff_len>>16)&0xff;
				temp_buff[CODE_LEN_ADDR+2] = (buff_len>>8)&0xff;
				temp_buff[CODE_LEN_ADDR+3] = (uint8_t)buff_len;
...
uint8_t RemoteUpDataByEth(uint8_t *buff)
{
	...
	if(crc_tmp2 == crc_tmp1)
	{
		...
		if(firstdata_flag)
		{
			if(next_pack == packetnum-1 || packetnum == 0)
			{
				...
				{
					#if USE_OFFCHIP_FLASH
						W25QXX_Write(&buff[2], UPDATA_ADDR + packetnum*(PLATFORM_BUFF_SIZE-4), PLATFORM_BUFF_SIZE-4);
					#else
				...
				{
					#if USE_OFFCHIP_FLASH
						W25QXX_Write(&buff[2], UPDATA_ADDR + packetnum*(PLATFORM_BUFF_SIZE-4), File_Size%(PLATFORM_BUFF_SIZE-4));
					#else
	...
	if(packetnum == (file_pack_num-1))		//接收完成所有数据包
	{
		...
		myprintf("\r\n 计算更新区内的程序MD5：");
		CheckCodeMD5_Boot(UPDATA, File_Size, calculate_md5);
		for(i=0;i<16;i++)
		{
			myprintf("%02x",calculate_md5[i]);
		}
		if(StrComplate(calculate_md5,fileMD5,16)==1)	//md5校验通过
		{
			...
			NVIC_SystemReset();//重启
			taskENTER_CRITICAL();
			vTaskDelete(RemoteUpdata_Task_Handle);	//删除任务
//			vTaskSuspend(RemoteUpdata_Task_Handle);//挂起任务
			taskEXIT_CRITICAL();
...
//	RemoteUpdata_buf = (uint8_t *)malloc(PLATFORM_BUFF_SIZE * sizeof(uint8_t));
	...
	while(1)
	{
        ...
        {
			RemoteUpDataByEth(RemoteUpdata_buf);
