...
/*
	用x轴与水平面夹角和Y轴与水平面夹角，计算xy平面与水平面夹角
	计算公式：arcsin
*/
static double tmp0,tmp1,tmp2,tmp3;
double Get_TiltAngle(double angle_x, double angle_y)
{
	tmp0 = cos((double)angle_x/180*PI)*cos((double)angle_x/180*PI) +\
					cos((double)angle_y/180*PI)*cos((double)angle_y/180*PI);
	tmp1 = tmp0-1;
	tmp2 = sqrt(tmp1);
	tmp3 = acos(tmp2);
	if(angle_x>=90 || angle_y>=90)
		return 90;
//	if((angle_x+angle_y) > 90)
////		return (90-asin(sqrt(cos((double)angle_x/180*PI)*sin((double)angle_x/180*PI) +\
////					sin((double)angle_y/180*PI)*sin((double)angle_y/180*PI)-1))*180/PI);
//		return acos(sqrt(cos((double)angle_x/180*PI)*cos((double)angle_x/180*PI) +\
//					cos((double)angle_y/180*PI)*cos((double)angle_y/180*PI)))*180/PI;
//	else
		return asin(sqrt(sin((double)angle_x/180*PI)*sin((double)angle_x/180*PI) +\
					sin((double)angle_y/180*PI)*sin((double)angle_y/180*PI)))*180/PI;
}
